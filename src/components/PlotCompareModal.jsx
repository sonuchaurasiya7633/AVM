    ...
    <>
      ...
      <AnimatePresence>
        {!isCompareModalOpen && comparedPlots.length > 0 && (
          ...
          >
            <div className="p-3 sm:px-5 sm:py-3.5 rounded-full bg-theme-surface/95 backdrop-blur-md border-2 dark:border-cyan-500/30 border-cyan-600/40 shadow-[0_15px_40px_rgba(0,0,0,0.35)] dark:shadow-[0_15px_40px_rgba(0,0,0,0.85)] flex items-center justify-between gap-4 text-theme-primary leather-stitch-outline">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 rounded-full text-theme-muted hover:text-theme-primary hover:bg-slate-200/50 dark:hover:bg-white/10 transition-colors"
      ...
      <AnimatePresence>
        {isCompareModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6 overflow-y-auto bg-black/80 dark:bg-black/90 backdrop-blur-md">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="relative w-full max-w-5xl rounded-3xl bg-theme-surface border-2 dark:border-cyan-500/30 border-slate-300 shadow-[0_25px_60px_rgba(0,0,0,0.5)] dark:shadow-[0_25px_60px_rgba(0,0,0,0.95)] overflow-hidden leather-stitch-outline my-auto flex flex-col max-h-[92vh]"
            >
              ...
              {/* Modal Header */}
              <div className="p-4 sm:p-6 border-b dark:border-luxury-gold/30 border-slate-200 flex items-center justify-between dark:bg-black/40 bg-slate-50/80">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base sm:text-xl font-serif font-bold text-theme-primary leading-snug">
                      {isHindi ? 'संस्थागत प्लॉट तुलनात्मक विश्लेषण' : 'Institutional Plotted Scheme Comparison Matrix'}
                    </h3>
                    <p className="text-[11px] text-luxury-gold font-mono font-medium">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={clearCompare}
                    className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold dark:bg-white/5 bg-slate-200/80 dark:border-white/20 border-slate-300 text-theme-secondary hover:text-theme-primary"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-full dark:bg-black/60 bg-slate-100 text-luxury-gold hover:text-theme-primary border border-luxury-gold/30"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
              ...
              <div className="flex-1 overflow-auto p-4 sm:p-6 scrollbar-thin scrollbar-thumb-luxury-gold/30">
                <div className="min-w-[650px]">
                  ...
                  >
                    ...
                    <div className="p-3 font-serif font-bold text-luxury-gold flex items-end">
                      <span>{isHindi ? 'मानक / विशेषता' : 'Statutory Metric'}</span>
                    </div>
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="relative rounded-2xl p-3 dark:bg-black/40 bg-slate-100/80 border border-luxury-gold/30 text-center">
                        <button
                          onClick={() => removeFromCompare(plot.id)}
                          className="absolute -top-2 -right-2 p-1 rounded-full bg-rose-700 dark:bg-rose-900 text-white border border-rose-500 hover:scale-110 transition-transform"
                          title="Remove"
                        >
                          <X className="w-3 h-3" />
                        </button>
                        ...
                        />
                        <h4 className="font-serif font-bold text-sm text-theme-primary truncate">
                          {plot.name}
                        </h4>
                        <span className="text-[11px] text-luxury-gold font-mono block font-semibold">
                          {plot.corridor}
                        </span>
                      </div>
                    ))}
                    ...
                    <div className="p-3 dark:bg-white/[0.02] bg-slate-100/60 rounded-xl text-xs font-semibold text-theme-secondary">
                      {isHindi ? 'शुरुआती दर (प्रति गज)' : 'Starting Rate / Gaj'}
                    </div>
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.02] bg-slate-100/60 rounded-xl text-xs font-serif font-bold text-gold-gradient text-center">
                        {plot.priceStartingGaj}
                      </div>
                    ))}

                    {/* Metric Row 2: Starting Ticket Size */}
                    <div className="p-3 dark:bg-white/[0.04] bg-slate-100/90 rounded-xl text-xs font-semibold text-theme-secondary">
                      {isHindi ? 'न्यूनतम कुल मूल्य' : 'Minimum Outlay'}
                    </div>
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.04] bg-slate-100/90 rounded-xl text-xs font-mono font-bold text-theme-primary text-center">
                    ...
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.02] bg-slate-100/60 rounded-xl text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 text-center flex items-center justify-center gap-1.5">
                    ...
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.04] bg-slate-100/90 rounded-xl text-xs font-mono text-center text-luxury-gold font-medium">
                    ...
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.02] bg-slate-100/60 rounded-xl text-xs font-mono text-theme-primary text-center">
                    ...
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.04] bg-slate-100/90 rounded-xl text-xs font-mono text-theme-primary text-center">
                    ...
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.02] bg-slate-100/60 rounded-xl text-xs font-mono text-emerald-600 dark:text-emerald-400 font-semibold text-center">
                    ...
                    {comparedPlots.map((plot) => (
                      <div key={plot.id} className="p-3 dark:bg-white/[0.04] bg-slate-100/90 rounded-xl text-xs font-mono font-bold text-luxury-gold text-center">
                        {isHindi ? 'तत्काल रजिस्ट्री व कब्जा' : 'Immediate Demarcation'}
                      </div>
                    ))}

                    {/* Action Row */}
                    <div className="p-3 text-xs font-semibold text-theme-secondary flex items-center">
